...
print(f"Need {needed_space} bytes")

big_dirs = sorted([x for x in directories if
    directories[x].size >= needed_space],
    key=lambda x: directories[x].size, reverse=True)

print(big_dirs[-1], directories[big_dirs[-1]].size)
